  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", position: "relative" }}>
      <nav 
        ...
        style={{
          width: barraLateralColapsada ? 80 : 260,
          flexShrink: 0,
          position: esMovil ? "fixed" : "relative",
          zIndex: esMovil ? 200 : 100,
          height: "100vh",
          display: esMovil && barraLateralColapsada ? "none" : "flex",
          flexDirection: "column",
          transition: "width .25s",
          padding: barraLateralColapsada ? "2rem 1rem" : "2rem",
          whiteSpace: "nowrap",
          overflow: "hidden"
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
          ...
          <button 
            onClick={() => setBarraLateralColapsada(!barraLateralColapsada)}
            style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div className={`nav-item ${pestanaActiva === 'panel' ? 'active' : ''}`} onClick={() => { setPestanaActiva('panel'); if(esMovil) setBarraLateralColapsada(true); }}>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid var(--glass-border)' }}>
          {!barraLateralColapsada && (
            <div className="user-info" style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {usuarioActual.name || usuarioActual.nombre} ({usuarioActual.role || usuarioActual.rol})
            </div>
          )}
          <button className="nav-item" style={{ width: '100%', border: 'none', background: 'transparent', textAlign: 'left', cursor: 'pointer', justifyContent: barraLateralColapsada ? 'center' : 'flex-start' }} onClick={manejarSalida}>
            <Iconos.Salir /> {!barraLateralColapsada && <span className="nav-text">Cerrar Sesión</span>}
          </button>
        </div>
      </nav>

      {/* OVERLAY PARA MÓVIL */}
      {esMovil && !barraLateralColapsada && (
        <div 
          onClick={() => setBarraLateralColapsada(true)}
      ...
      <main style={{
        ...
        {esMovil && (
          <div style={{ display: 'flex', padding: '12px', background: 'var(--glass-bg)', borderBottom: '1px solid var(--glass-border)', alignItems: 'center' }}>
            <button 
              onClick={() => setBarraLateralColapsada(false)}
